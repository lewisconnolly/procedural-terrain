...
bool Shader::AddTimeCBuffer(ID3D11Device* device, ID3D11DeviceContext* context, float time)
{
	// Supply the vertex shader constant data.
	TimeBufferType timeBufferData;

	timeBufferData.time = time;
	timeBufferData.padding = SimpleMath::Vector3().One;	

	// Fill in a buffer description.
	D3D11_BUFFER_DESC cbDesc;
	cbDesc.ByteWidth = sizeof(timeBufferData);
	...
	InitData.pSysMem = &timeBufferData;
	InitData.SysMemPitch = 0;
	InitData.SysMemSlicePitch = 0;

	// Create the buffer.
	HRESULT result = device->CreateBuffer(&cbDesc, &InitData, &m_timeBuffer);

	if (result != S_OK)
		return false;

	// Set time buffer in vertex shader
	context->VSSetConstantBuffers(1, 1, &m_timeBuffer);
